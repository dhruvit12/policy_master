...
class Perils_type extends BaseController
{
  ...
  public function index()
  {
   ...
   $data['page']='Perils_master/list';
      // print_r($data); exit;
   $M_vehicle_type = new Models\PerilsModel();
...
public function vehicle_model()
{
  ...
  $data['page']='Perils_master/vehicle_model';
  $M_vehicle_maker = new Models\Vehicle_MakerModel();
  $data['vehicle_maker'] = $M_vehicle_maker->where(['is_deleted'=>0])->findAll();
  ...
  $data['vehicle_model'] = $M_vehicle_model->where(['vehicle_maker.is_deleted'=>0])->findAll();
  echo view('templete',$data);
}
...
public function store_vehicle()
{
  ...
  return redirect()->to(site_url('Perils_type'));
}
...
{
  $data['datas']=array('perils_type'=>$_POST['perils_type'],'description'=>$_POST['description']);
  $M_user_role = new Models\PerilsModel();
  $M_user_role->like('perils_type_name',$_POST['perils_type']);
  $M_user_role->like('description',$_POST['description']);
  $data['list'] = $M_user_role->where(['is_deleted'=>0,'is_active'=>1])->findAll();
  $data['page']='Perils_master/list';
  echo view('templete',$data);
}
}
